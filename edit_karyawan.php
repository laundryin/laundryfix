...
if(isset($_POST['update'])) {
    ...
    $update = mysqli_query($conn, "UPDATE karyawan SET nama_karyawan='$nama', jabatan='$jabatan' WHERE id_karyawan='$id'");

    if($update) {
        echo "<script>alert('Data karyawan berhasil di-update!'); window.location='karyawan.php';</script>";
    }
}
?>
<!DOCTYPE html>
...
<head>
    <meta charset="UTF-8">
    <title>Edit Karyawan - Harum Laundry</title>
...
<body class="p-5">
    <div class="container" style="max-width: 600px;">
        <div class="card p-4">
            <h3 class="fw-bold mb-4 text-primary">Edit Data Karyawan</h3>
            <form method="POST">
                <div class="mb-3">
                    <label class="form-label">Nama Karyawan</label>
                    <input type="text" name="nama_karyawan" class="form-control rounded-pill" value="<?= $d['nama_karyawan']; ?>" required>
                </div>
                <div class="mb-3">
                    <label class="form-label">Jabatan</label>
                    <input type="text" name="jabatan" class="form-control rounded-pill" value="<?= $d['jabatan']; ?>" required>
                </div>
